...
// @Service는 보다 구체화된 즉, Service의 의미를 가지는 클래스라는걸 보여주기 위한
...
public class MemberServiceImpl implements MemberService {
	...
	public Member loginMember(Member m) {
		...
		// 내가 입력한 비밀번호를 다시 암호화해서 암호화한 값과
		// DB의 비번값을 비교해서 일치하면 로그인 성공
		/*String encPwd = bcryptPasswordEncoder.encode(m.getPwd());
		m.setPwd(encPwd);*/
		
		// 매번 랜덤한 솔트값을 가지고 암호화를 하므로 encode() 호출 시마다 다이제스트 값은 변경됨
		// 스프링 시큐리티에서 제공하는 matches() 메소드를 통해 원문과 평문이 일치하는지 비교 가능
		
		// => 사용자가 입력한 id만으로 DB에서 조회해온 뒤
		// 조회된 pwd와 사용자가 입력한 pwd가 matches() 메소드를 통과하는지 확인하여
		// 로그인 처리를 완료함
		Member loginUser = mDao.selectMember(m);
		
		// select된 아이디 값은 있으나 비밀번호가 매칭되지 않은 경우
	...
	public int insertMember(Member m) {
		...
		// => 이를 극복하기 위해 salting 이라는 기법이 추가
		// 원본 메시지 + salt를 추가하여 동일한 길이의 다이제스트를 생성하는 방법
