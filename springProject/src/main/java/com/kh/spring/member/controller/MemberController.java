...
public class MemberController {
	...
	// 파라미터를 전송받는 방법
	/* 1. HttpServletRequest를 통해 전송받기(기존 방식)
	 ...
	 * HttpServlet과 비슷하게 request 객체를 이용하여 데이터를 전송받는 방법
	 * 
	 */
	/*@RequestMapping(value="login.do", method=RequestMethod.POST)
	 ...
	 * (단, 매개변수를 name 값과 동일하게 해야 자동으로 값이 주입된다.) 
	 * 
	 * 다만 어노테이션을 생략할 경우 다른 설정값은 불가
	 * */
	/*@RequestMapping(value="login.do", method=RequestMethod.POST)
	 ...
	 * (주의 : 반드시 name 속성의 값과 필드명을 동일시켜야 하며 setter 규칙에 맞추어 작성해야 함)
	 * 
	...
	/* 5. @ModelAttribute 어노테이션 생략하고 객체로 바로 전달받는 방법
	 * 
	 * 어노테이션을 생략해도 자동으로 커맨드 객체로 매핑이 된다.
	 * 
	 * 스프링에서 파라미터를 전달하는 방법은 상당히 다양하다
	 * 이외에도 @RequestBody라는 어노테이션으로 요청하는 데이터가
...
//	public String memberLogin(Member m, HttpSession session) {
		 ...
		 * 		의존성 주인을 통해 결합 관계에 있는 두 클래스의 의존 관계를 해결하게 만들어준다.
		 * 
		 * 2번 --> 매 요청마다 같은 해쉬코드 값을 확인할 수 있다
		 * 		즉, 한 개의 객체만 생성해 놓고 계속 사용하는 것이다.
		 * 		싱글톤 개념 -> 메모리의 효율이 올라간다.
		 * 		요청 10000건을 처리할 때 mService 10000개의 객체를 필요로 하는 것이 아니라
		 * 		1개의 객체만 관리하면 된다.
		*/
...
//	}
	...
	// 이제는 전달받는 것에 대한 것이 아니라 요청 후 전달하고자 하는 데이터가 있을 경우의 처리
	// 1. Model 객체를 사용하는 방법
	// 커맨드 객체로 Model을 사용하게 되면 뷰로 전달하고자 하는 데이터를 맵 형식(key, value)
	// 로 담을 수 있다. scope는 request이다. (서블릿에서는 사용하던 requestScope와 동일)
	/*@RequestMapping(value="login.do", method=RequestMethod.POST)
	 ...
	 * View : requestDispathcer의 forward 시 이동할 페이지의 정보(url)을 담는 객체
	 * 
	 * Model은 따로 사용 가능하지만 View는 단독 사용 불가능
	 * 		-> 대신 String 변환형으로 url 입력 시 forward 동작을 함.
	 */
	/*@RequestMapping(value="login.do", method=RequestMethod.POST)
	...
	public String  memberLogin(Member m, Model model) { 
		...
		if(loginUser != null) {
			...
			// RuntimeException으로 상속받았을 때 차이점
			// -> throws를 넘길 필요 없으며 try-catch로 잡을 필요도 없다
			
			// 에러페이지로 연결하는 방법 -> web.xml에 공용 에러페이지를 등록하여
	 ...
	 * 새로 요청을 보내 새로운 request, response 객체를 생성하고 주소값이 바뀜.
	 * */
	...
	public String memberInsert(Member m, 
		...
		// 한글 인코딩은 스프링에서 제공하는 필터를 이용해서 간단하게 처리할 수 있다.
		// web.xml로 이동!
		...
		// RedirectAttributes : Redirect 시 데이터를 전달할 수 있는 객체
		// RedirectAttributes.addFlashAttribute()
		// : Redirect로 데이터 전달시 URL에 데이터가 노출되지 않게 해주는 메소드
		if(result > 0) {
	...
	// 수정하기
	// 성공시 msg 회원정보가 수정 되었습니다 -> home으로 이동
	// 실패시 msg 회원정보 수정 실패 -> errorPage로 이동
	...
	// 삭제하기
	// 성공시 msg 회원정보가 삭제 되었습니다 -> home으로 이동(+ 로그아웃 처리)
	// 실패시 msg 회원 삭제 실패 -> erroePage로 이동
